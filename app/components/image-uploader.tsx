...
export const ImageUploader = ({ onChange, imageUrl }: props) => {
  const [draggingOrder, setDraggingOver] = useState(false);
  ...
  return (
    <div
      className={`${draggingOrder ? 'border-4 border-dashed border-yellow-300 border-rounded' : ''} group rounded-full relative w-24 h-24 flex justify-center items-center bg-gray-400 transition duration-300 ease-in-out hover:bg-gray-500 cursor-pointer`}
      style={{
